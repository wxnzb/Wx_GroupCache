...
//
// 这个是调用远程节点是用的
func (f *fackPeer) Get(request *pb.GetRequest, response *pb.GetResponse) error {
	f.hits++
	if f.fail {
...
func TestPeers(t *testing.T) {
	...
	localHits := 0
	//这个是本地调用
	get := func(key string, dst Sink) error {
		localHits++
	...
	run := func(name string, n int) {
		...
		for i := 0; i < n; i++ {
			...
			want := "got:" + key
			var got string
			if err := testGroup.Get(key, StringSink(&got)); err != nil {
				t.Fatal(err)
			}
			if got != want {
				t.Fatal("bad value")
...
func TesttruncBytesSink(t *testing.T) {
	var buf [100]byte
	s := buf[:]
	if err := StringGroup.Get("wxnzb", TruncBytesSink(&s)); err != nil {
		t.Fatal(err)
	}
